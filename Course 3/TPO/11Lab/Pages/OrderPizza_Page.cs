...
namespace _11Lab
{
    public class Order_DodoPizzaPage
    {
        ...
        private By cardNumberInput = By.XPath("//*[@id='card-number']");
        private By cardCvcInput = By.XPath("//*[@id='card-cvc']");
        private By cardMonthYearInput = By.XPath("//*[@id='card-month-and-year']");

        public Order_DodoPizzaPage(IWebDriver driver)
        {
            this.driver = driver;
        ...
        public void EnterCardDetails(string cardNumber, string cardCvc, string cardMonthYear)
        {
            Thread.Sleep(5000);
            driver.FindElement(cardNumberInput).SendKeys(cardNumber);
            Console.WriteLine("Введен номер карты: " + cardNumber);
            driver.FindElement(cardCvcInput).SendKeys(cardCvc);
            Console.WriteLine("Введен CVC код: " + cardCvc);
            driver.FindElement(cardMonthYearInput).SendKeys(cardMonthYear);
            Console.WriteLine("Введена дата: " + cardMonthYear);
        }
    }
}
